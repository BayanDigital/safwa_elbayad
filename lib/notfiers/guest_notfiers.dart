...
class ProductsNotifier extends StateNotifier<ApiState<ProductsModel>> {
  ...
  Future<void> getAllVariations() async {
    if (!mounted) return; // تحقق قبل بدء التنفيذ

    state = const ApiState.loading();

    try {
      final result = await repo.getProducts(data.servieID, data.variationID);
      if (!mounted) return; // تحقق مرة أخرى بعد await

      state = ApiState.loaded(data: result);
    } catch (e) {
      if (!mounted) return; // تحقق قبل تعيين الخطأ
...
class ProductsNotifierData extends StateNotifier<ApiState<ProductsModel>> {
  ProductsNotifierData(this.repo, this.data) : super(const ApiState.initial()) {
    getAllVariations();
  }
  ...
  Future<void> getAllVariations() async {
    ...
    try {
      ...
      // debugPrint("getprstacktrace $stackTrace");
      // debugPrint("getpro $e");
      // throw stackTrace;
